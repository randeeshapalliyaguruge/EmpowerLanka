    ...
    <p>User ID: {{ $product->user_id }}</p> --}}
        ...
        <div class="container mx-auto">
            <div class="bg-white rounded-lg shadow-lg pl-20 my-3">
                <h3 class="text-3xl font-semibold text-gray-800 pl-8 pt-9">{{ $featured->title }}</h3>
                <p class="text-gray-600 text-lg mb-2 p-8">Posted: {{ $featured->created_at }} , {{$featured->user->city}}, {{$featured->user->province}}</p>
                <div class="flex flex-wrap pl-10">
                    <div class="w-full sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2 pl-8 pb-8">
                        <img src="/storage/{{ $featured->image }}" alt="Advertisement Image" class="w-full h-auto rounded-lg">
                    </div>
                        ...
                        <div class='w-full max-w-md  mx-auto bg-white rounded-3xl shadow-xl overflow-hidden'>
                            <div class='max-w-md mx-auto'>
                              <div class='p-4 sm:p-6'>
                                <p class='font-bold text-gray-700 text-[22px] leading-7 mb-1'>Seller Details: {{ $featured->user->name }}</p>
                                <div class='flex flex-row'>
                                  <p class='text-[17px] font-bold text-[#0FB478]'>Rs. {{ number_format($featured->price, 0, ',', ',') }}</p>
                                </div>
                                <p class='text-[#7C7C80] font-[15px] mt-6'>Last Edited: {{ $featured->updated_at }}</p>
                                <p class='text-[#7C7C80] font-[15px] mt-1'>Views: {{ $featured->category->name }}</p>
                                <p class='text-[#7C7C80] font-[15px] mt-1'>Views: {{ $featured->view_count }}</p>
                                <p class='text-[#7C7C80] font-[15px] mt-1'>Status: {{ $featured->status ? 'Active' : 'Inactive' }}</p>

                                <p class='text-[#7C7C80] font-[15px] mt-6' id='phone-number' style='display: none; color: #0FB478; font-weight: bold'>Phone Number: {{ $featured->number }}</p>
                                <p class='text-[#7C7C80] font-[15px] mt-1' id='email' style='display: none; color: #0FB478; font-weight: bold'>Email: {{ $featured->user->email }}</p>
                                ...
                                <div id='edit-delete-buttons' style='display: none' class="mt-4 ml-2">
                                  ...
                                  <a href="{{ auth()->user()->hasRole('admin') ? route('admin.featured.edit', $featured->id) : route('user.featured.edit', $featured->id) }}">
                                    <x-table-action-button>Edit</x-table-action-button>
                                  </a>
                                  @endif
                                  <form action="{{ auth()->user()->hasRole('admin') ? route('admin.featured.destroy', $featured) : route('user.featured.destroy', $featured) }}" method="POST"
                                    onsubmit="return confirm('Are you sure you want to delete this featured ad?');">
                                    @csrf
                                    @method('DELETE')
